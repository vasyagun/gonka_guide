...
- Blackwell (RTX 50xx / sm120, e.g. 5060/5070/5080/5090)
  - Image: `vasyagun/gonka:mlnode-3.0.11-clore-sm120-post15`
  - Use on Blackwell GPUs only.

- Ada / Ampere (RTX 40xx / 30xx, e.g. 4090/3090)
  - Image: `vasyagun/gonka:mlnode-3.0.11-clore-ada-post15`
...
- `GET /api/v1/logs/vllm?lines=200&follow=1` (stream)
- `GET /api/v1/logs/uvicorn?lines=200&follow=1` (stream)
- `GET /api/v1/logs/preload?lines=200&follow=1` (stream)
- `GET /api/v1/diag`
...
Realtime tail (no SSH):
```
curl -N http://<HOST>:<PORT>/api/v1/logs/vllm?lines=50&follow=1
```
...
- Image: use one of the `post15` images above.
- Port forwarding:
  - `22/TCP` (SSH)
  - `5000/TCP` (public API)
- Environment:
...
Recommended flow:
1) Wait for model preload to finish (or disable preload).
2) Verify state and diagnostics.
3) Register the node in the network node.
4) Stream logs to confirm inference startup.
...
IMPORTANT:
- Do not register the node until preload is complete (check `/api/v1/logs/preload`).
- If you register too early, inference may not start in time for the epoch.
...
After registration, stream logs to confirm startup:
```
curl -N "http://<HOST>:<PORT>/api/v1/logs/uvicorn?lines=200&follow=1"
curl -N "http://<HOST>:<PORT>/api/v1/logs/vllm?lines=50&follow=1"
```
